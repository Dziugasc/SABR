...
-- This is the size of the Cubes - change it however you want, the rest will fit - always!
 ...
 settings.SizeX = settings.boxsize * 10
 settings.SizeY = settings.boxsize * 18
...
stretch = settings.SizeY / 288

cleftSizeX, cleftSizeY = 32 * stretch, 288 * stretch
crightSizeX, crightSizeY = 128 * stretch, 288 * stretch

totalXSize = settings.SizeX + ( leftSizeX + rightSizeX ) * stretch
totalYSize = settings.SizeY
...
function startTetris ( cmd, diff )
	...
	if isElement ( gImage["Back"] ) then
		outputChatBox ( "You can just start one game at the same time!", 125, 0, 0 )
	else
		local diff = tonumber ( diff )
		if diff then
			level = diff
			speed = 1100 - level * 100
			createTetrisField ( nil )
			outputChatBox ( "Type /stoptetris to stop the game - Have fun!", 125, 0, 0 )
		else
			outputChatBox ( "Please use /tetris [1-10]", 125, 0, 0 )
		end
	end
end
addCommandHandler ( "tetris", startTetris )
...
addCommandHandler ( "stoptetris", stoptetris_func )
...
function createTetrisField ( parent )
	...
	music = playSound ( "sound/tetris.mp3", true )
	...
	gImage["Left"] = guiCreateStaticImage ( 0, 0, leftSizeX * stretch, settings.SizeY, "images/left.bmp", false, gImage["Back"] )
	gImage["Center"] = guiCreateStaticImage ( leftSizeX * stretch, 0, settings.SizeX, settings.SizeY, "images/white.bmp", false, gImage["Back"] )
	gImage["Right"] = guiCreateStaticImage ( settings.SizeX + leftSizeX * stretch, 0, rightSizeX * stretch, settings.SizeY, "images/right.bmp", false, gImage["Back"] )
	...
	gImage["showbox"] = guiCreateStaticImage ( 0.39, 0.73, (settings.boxsize*4)/(rightSizeX * stretch), (settings.boxsize*4)/settings.SizeY, "images/white.bmp", true, gImage["Right"] )
...
function showNextBlockInfo()
	...
	if isElement ( gImage["showbox"] ) then
		...
		for i = 1, 4 do
			...
			local x, y = (x-1)*(settings.boxsize), (y-1)*(settings.boxsize)
			local width, height = settings.boxsize, settings.boxsize
...
function checkForGameOver()
	...
	if sucess then
		...
		destroyElement ( music )
		setTimer ( destroyElement, 5000, 1, gImage["Back"] )
...
function drawBlock ( x, y, model )
	...
	if isElement ( gImage["Center"] ) then
		px, py = ( x - 1 ) * settings.boxsize, ( y - 1 ) * settings.boxsize
		path = models[model]
		fields[x][y] = guiCreateStaticImage ( px, py, settings.boxsize, settings.boxsize, path, false, gImage["Center"] )
	end
end
